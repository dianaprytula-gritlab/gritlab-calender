...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "A friendlier calendar for grit:lab events — raids, checkpoints, fika, and more.",
      },
...
function Index() {
  ...
  const [view, setView] = useState<"month" | "agenda">(() => {
    if (typeof window !== "undefined" && window.location.hash.toLowerCase() === "#agenda")
      return "agenda";
    return "month";
  });
  ...
  const upcoming = useMemo(
    () =>
      EVENTS.filter((e) => active.has(e.category) && new Date(e.end).getTime() >= now.getTime())
      ...
      <header className="border-b bg-card/60 backdrop-blur sticky top-0 z-30">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="size-9 rounded-xl bg-primary text-primary-foreground grid place-items-center font-display text-lg shadow-sm">
              g:
            </div>
            <div>
              ...
              {upcoming.map((e) => {
                ...
                return (
                  <li key={e.id} className="flex items-start gap-3">
                    <div
                      className={`mt-1 size-8 rounded-lg ${t.soft} ${t.text} grid place-items-center text-base shrink-0`}
                    >
                      {meta.emoji}
                    </div>
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-muted-foreground">
                        {new Date(e.start).toLocaleDateString("en", {
                          weekday: "short",
                          month: "short",
                          day: "numeric",
                        })}
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex gap-2 text-xs">
              <button
                onClick={() => setActive(new Set(ALL_CATS))}
