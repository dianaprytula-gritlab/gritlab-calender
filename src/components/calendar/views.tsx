...
function inRange(day: Date) {
  const d = new Date(day);
  d.setHours(0, 0, 0, 0);
  const rs = new Date(RANGE_START);
  rs.setHours(0, 0, 0, 0);
  const re = new Date(RANGE_END);
  re.setHours(0, 0, 0, 0);
...
export function MonthView({ active }: { active: Set<CategoryKey> }) {
  ...
  const cells = useMemo(() => {
    ...
    // Adjust start to the Sunday of that week to align with column headers
    ...
    <div className="rounded-2xl border bg-card shadow-sm overflow-hidden">
      <div className="flex items-center justify-between px-5 py-4 border-b bg-gradient-to-r from-background to-card">
        <h2 className="font-display text-2xl">
          May 21 <span className="text-muted-foreground">–</span> Jun 3{" "}
      ...
      <div className="grid grid-cols-7 border-b bg-muted/30">
        {DAYS.map((d) => (
          <div
            key={d}
            className="px-3 py-2 text-xs font-semibold tracking-widest uppercase text-muted-foreground"
          >
            {d}
          </div>
        ))}
